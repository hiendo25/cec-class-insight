...
export function MonthlyHub() {
  ...
  const dong = useMemo(() => {
    ...
    for (const c of CLASSES) {
      ...
      if (tong === 0) continue;
      ra.push({ id: c.id, code: c.code, campus: c.campus, mine: c.mine, tong, duyet, nhap });
    }
    /* lớp còn nhiều việc lên trước */
    return ra.sort((a, b) => (b.tong - b.duyet) - (a.tong - a.duyet));
  }, [thang]);

  const list = chiCuaToi ? dong.filter((d) => d.mine) : dong;
  ...
  return (
    <div className="flex flex-col gap-[13px]">
      ...
      {list.length === 0 ? (
        ...
        <div className="cec-scroll overflow-x-auto rounded-[8px] bg-white" style={{ border: `1px solid ${LINE}` }}>
          <table className="border-collapse text-[13px]" style={{ minWidth: 760, width: "100%" }}>
            <colgroup>
              {[150, 170, 96, 110, 180, 120].map((w, i) => <col key={i} style={{ width: w }} />)}
            </colgroup>
            <thead>
              <tr style={{ background: TH_BG, color: TH_FG, borderBottom: `1px solid ${TH_LINE}` }}>
                {["Lớp học", "Cơ sở", "Số em", "Đã duyệt", "Tiến độ", ""].map((h) => (
            ...
            <tbody>
              {list.map((d, i) => {
                ...
                return (
                  <tr key={d.id} style={{ background: i % 2 ? "#f5f8fc" : "#fff", borderBottom: "1px solid #edeff4" }}>
      ...
      <p className="text-[12px]" style={{ color: INK3 }}>
        Bấm một lớp để mở màn duyệt hàng loạt — nơi xem ba cờ đủ nguồn / thiếu nguồn / nghi trùng tháng trước.
      </p>
    </div>
  );
}
